...
class Day14(input: Input) : Puzzle() {
    ...
    private fun setMemWithMaskedValue(setMask: SetMask, setMem: SetMem) =
        listOf(SetMem(setMem.address, (setMem.value and setMask.xMask) or setMask.nMask))
    ...
    private fun setMemPermutations(mask: SetMask, instruction: SetMem) =
        (instruction.address and mask.xMask.inv())
            .let { fixedBits ->
                generateSequence(mask.xMask) { if (it > 0) (it - 1) and mask.xMask else null }
                    .map { fixedBits or mask.nMask or it }
                    .toList()
            }
            .map { SetMem(it, instruction.value) }

    private fun sumOfMemory(setMemInstructions: (SetMask, SetMem) -> List<SetMem>) = program
        .runningFold(SetMask(0, 0) to listOf<SetMem>()) { (mask, previous), next ->
            when (next) {
